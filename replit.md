...
## Recent Changes
- 2026-02-06: Backend-only time analysis - All question endpoints (generate, regenerate, refine-selected) run AI time analysis inline. Questions come back with estimatedMinutes and isMandatory already set. Frontend no longer auto-triggers a separate analysis call. The analyze-time endpoint is kept only as a manual retry fallback. Single-question refinement preserves original time estimate and inclusion status.
- 2026-02-06: Removed frontend auto-analysis useEffect - Previously, frontend had a useEffect that auto-triggered analyze-time endpoint when questions lacked estimates. This caused a redundant second analysis call. Now the frontend simply detects whether questions have estimates on load and sets aiAnalysisComplete accordingly.
