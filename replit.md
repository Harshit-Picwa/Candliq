...
## Recent Changes
- 2026-02-07: Optimized question generation speed - Removed separate analyzeQuestionTime calls from generate, regenerate, and refine-selected endpoints. The AI generation prompt already produces estimatedMinutes and isMandatory values, so the second analysis call was redundant. This cuts generation time roughly in half (1 AI call instead of 2). The analyze-time endpoint is kept only as a manual retry/re-analysis fallback.
- 2026-02-06: Single-question refinement preserves original time estimate and inclusion status instead of re-analyzing.
- 2026-02-06: Removed frontend auto-analysis useEffect. Frontend detects whether questions have estimates on load and sets aiAnalysisComplete accordingly.
