...
## Recent Changes
- 2026-02-06: Inline time analysis - All question endpoints (generate, regenerate, refine-question, refine-selected) now run AI time analysis inline after generation/refinement. Questions come back with estimatedMinutes and isMandatory already set by AI. If inline analysis fails, questions save without estimates and frontend auto-triggers analysis as fallback. Frontend no longer needs a separate analysis step in most cases.
- 2026-02-06: Pending Analysis workflow - Generated questions start in "pending" state (isMandatory: false, no estimatedMinutes). Frontend auto-triggers AI analysis via analyze-time endpoint which uses Gemini thinking mode to independently decide which questions to include based on time budget. UI shows "AI is analyzing..." banner during analysis, hides complexity badges and time estimates until analysis completes. Failed analysis shows retry button instead of faking completion. State resets on regenerate/refine to retrigger analysis.
- 2026-02-06: AI Time Analysis uses Gemini 2.5 Pro with thinking mode via direct REST API (thinkingBudget: 8192) for independent, accurate time estimates. The @google/generative-ai SDK doesn't support thinkingConfig, so REST API is used specifically for this feature. Response parsing filters out thought parts (p.thought === true), extracts text parts, with fallback to generateTextWithRetries if REST fails. After analysis, updates stored estimates so frontend stays in sync.
- 2026-02-06: AI prompts updated to maximize mandatory questions that fit within screening time. All three prompt builders (main, first-batch, subsequent-batch) now instruct AI to pack as many mandatory questions as possible into the time budget (targeting 3-4 min each), then add 2-3 optional extras. The AI decides mandatory vs optional based on whether questions fit within the screening time.
